from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar, Union

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.membership_level_id import MembershipLevelId


T = TypeVar("T", bound="MemberShipCustomFieldData")


@_attrs_define
class MemberShipCustomFieldData:
    """
    Attributes:
        membership_levels (Union[Unset, list['MembershipLevelId']]):
    """

    membership_levels: Union[Unset, list["MembershipLevelId"]] = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        membership_levels: Union[Unset, list[dict[str, Any]]] = UNSET
        if not isinstance(self.membership_levels, Unset):
            membership_levels = []
            for membership_levels_item_data in self.membership_levels:
                membership_levels_item = membership_levels_item_data.to_dict()
                membership_levels.append(membership_levels_item)

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if membership_levels is not UNSET:
            field_dict["membershipLevels"] = membership_levels

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.membership_level_id import MembershipLevelId

        d = dict(src_dict)
        membership_levels = []
        _membership_levels = d.pop("membershipLevels", UNSET)
        for membership_levels_item_data in _membership_levels or []:
            membership_levels_item = MembershipLevelId.from_dict(membership_levels_item_data)

            membership_levels.append(membership_levels_item)

        member_ship_custom_field_data = cls(
            membership_levels=membership_levels,
        )

        member_ship_custom_field_data.additional_properties = d
        return member_ship_custom_field_data

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
